...
public class WXController {
    ...
    public CommonResult loginByWeixin(@RequestBody LoginInfo loginInfo, HttpServletRequest request) {
        ...
        CommonResult commonResult = memberService.wx_login(openid);

        if (commonResult == null) {
            //generate a telephone number
            String tel = String.valueOf(System.currentTimeMillis());
            memberService.wx_register(openid, loginInfo.getNickName(),loginInfo.getAvatarUrl(),loginInfo.getCity(),loginInfo.getGender(), tel);
            commonResult = memberService.wx_login(openid);
        }
        this.LOGGER.info("common result:"+ commonResult.getData());
